...
function updateModalTranslations() {
    ...
    if (telegramModal) {
        ...
        const enableLabel = telegramModal.querySelector('label[for="telegramEnabled"]');
        const notifyTitle = telegramModal.querySelector('.notification-settings h4');
        const completeLabel = telegramModal.querySelector('label[for="notifyPrintComplete"]');
        const errorLabel = telegramModal.querySelector('label[for="notifyPrintError"]');
        const pausedLabel = telegramModal.querySelector('label[for="notifyPrintPaused"]');
        const offlineLabel = telegramModal.querySelector('label[for="notifyPrinterOffline"]');
        const onlineLabel = telegramModal.querySelector('label[for="notifyPrinterOnline"]');
        const inefficiencyLabel = telegramModal.querySelector('label[for="notifyInefficiency"]');
        const inefficiencyReasonLabel = telegramModal.querySelector('label[for="notifyInefficiencyReason"]');
        const programStartLabel = telegramModal.querySelector('label[for="notifyProgramStart"]');
        ...
        if (enableLabel) enableLabel.textContent = t('enable_notifications');
        if (notifyTitle) notifyTitle.textContent = t('notify_on');
        if (completeLabel) completeLabel.textContent = t('print_complete');
        if (errorLabel) errorLabel.textContent = t('print_error');
        if (pausedLabel) pausedLabel.textContent = t('print_paused');
        if (offlineLabel) offlineLabel.textContent = t('printer_offline');
        if (onlineLabel) onlineLabel.textContent = t('printer_online');
        if (inefficiencyLabel) inefficiencyLabel.textContent = t('inefficiency_event');
        if (inefficiencyReasonLabel) inefficiencyReasonLabel.textContent = t('inefficiency_reason_saved');
        if (programStartLabel) programStartLabel.textContent = t('program_start');
...
function addPrinter() {
    ...
    const port = portInput ? portInput.value.trim() : '7125';
    const webPort = webPortInput ? webPortInput.value.trim() : '80';
    ...
    addConsoleMessage(`✅ ${t('printer_added')} ${name} (${ip}:${port})`, 'info');
}
...
function editPrinter(printerId, event) {
    ...
    if (portInput) portInput.value = printer.port;
    ...
    container.innerHTML = printers.map(printer => `
        <div class="printer-card ${printer.status}" data-printer-id="${printer.id}" onclick="openPrinterWebInterface('${printer.id}')">
            ...
            <div class="printer-info">
                <div class="info-item">
                    <span>${t('address')}</span>
                    <span class="ip-address-large">${printer.ip}</span>
                </div>
                <div class="info-item">
                    <span>${t('state')}</span>
