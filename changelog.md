...
### 📹 ИСПРАВЛЕНА ПОДДЕРЖКА МОДЕЛЕЙ ДЛЯ КАМЕРЫ!

#### Важное открытие:
- **bambu-js поддерживает камеру только для P1S и H2D**
- **X1C и A1 НЕ поддерживаются** библиотекой bambu-js для камеры
- **Это объясняет почему камера не работала** на X1C и A1 принтерах

#### Поддерживаемые модели камеры:
- ✅ **P1S** - полная поддержка через TCP_STREAM (порт 6000)
- ✅ **H2D** - полная поддержка через RTSP (порт 322)
- ❌ **X1C/X1-Carbon** - НЕ поддерживается bambu-js
- ❌ **A1/A1-mini** - НЕ поддерживается bambu-js

#### Исправления:
- **Умная проверка моделей**: Используется `isSupportedModel()` из bambu-js
- **Правильная логика**: Камера показывается только для поддерживаемых моделей
- **HTTP fallback**: Для неподдерживаемых моделей пробуются HTTP методы
- **Детальное логирование**: Чёткие сообщения о поддержке моделей

#### Техническая информация:
- **P1S**: TCP_STREAM на порту 6000, username: 'bblp'
- **H2D**: RTSP на порту 322, path: '/streaming/live/1'
- **X1C/A1**: Попытка HTTP методов как fallback

#### Результат:
- 📹 **Камера работает только на P1S и H2D** (как и задумано в bambu-js)
- 🔍 **Чёткая диагностика** - логи показывают почему камера недоступна
- ⚡ **Нет ложных попыток** подключения к неподдерживаемым моделям
- 🎯 **Правильное поведение** для каждой модели принтера
...
## 🔧 v1.5.19 (09.10.2025) - Camera Always Enabled + CSP Fix
